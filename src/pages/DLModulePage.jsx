...
import DL_PROJECTS from '../data/dlProjects';
import { useProgress } from '../context/ProgressContext';
import MarkdownRenderer from '../components/ui/MarkdownRenderer';
import MiniProject from '../components/ui/MiniProject';

export default function DLModulePage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const moduleId    = parseInt(id, 10);
  const moduleIndex = DL_MODULES.findIndex((m) => m.id === moduleId);
  const mod         = DL_MODULES[moduleIndex];
  const projects    = DL_PROJECTS[moduleId] || [];
  ...
  if (!mod) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/dl" className="btn-secondary">← Retour à la roadmap Deep Learning</Link>
      </div>
    );
  }
  ...
  const prevModule       = moduleIndex > 0 ? DL_MODULES[moduleIndex - 1] : null;
  const nextModule       = moduleIndex < DL_MODULES.length - 1 ? DL_MODULES[moduleIndex + 1] : null;
  ...
  return (
    <div className="animate-fade-in">
      ...
      >
        ← Retour à la roadmap Deep Learning
      </Link>
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <span className="text-xs font-mono font-bold uppercase tracking-wider" style={{ color: mod.colorHex }}>
            {mod.level} — Module {moduleIndex + 1}/{DL_MODULES.length}
          </span>
      ...
      {projects.length > 0 && (
        <section className="mb-10">
          ...
          <p className="text-sm text-text-muted mb-6">
            Applique les concepts sur des datasets réels. Réalise chaque projet sur ta machine, puis marque-le comme terminé.
          </p>
      ...
      <div className="flex justify-between items-center pt-6 border-t border-surface-3/30">
        {prevModule ? (
          <button onClick={() => navigate(`/dl/module/${prevModule.id}`)} className="btn-secondary text-sm">
            ← {prevModule.title}
          </button>
        ) : <div />}
        {nextModule ? (
          <button onClick={() => navigate(`/dl/module/${nextModule.id}`)} className="btn-primary text-sm">
            {nextModule.title} →
          </button>
        ) : (
          <Link to="/dl" className="btn-primary text-sm">🎉 Parcours terminé !</Link>
        )}
      </div>
    </div>
  );
