...
export default function PythonModulePage() {
  ...
  const moduleIndex = PYTHON_MODULES.findIndex((m) => m.id === moduleId);
  const mod         = PYTHON_MODULES[moduleIndex];
  const projects    = PYTHON_PROJECTS[moduleId] || [];
  ...
  if (!mod) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/python" className="btn-secondary">← Retour à la roadmap Python</Link>
      </div>
    );
  }
  ...
  const prevModule       = moduleIndex > 0 ? PYTHON_MODULES[moduleIndex - 1] : null;
  const nextModule       = moduleIndex < PYTHON_MODULES.length - 1 ? PYTHON_MODULES[moduleIndex + 1] : null;
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <span className="text-xs font-mono font-bold uppercase tracking-wider" style={{ color: mod.colorHex }}>
            {mod.level} — Module {moduleIndex + 1}/{PYTHON_MODULES.length}
          </span>
      ...
      {projects.length > 0 && (
        <section className="mb-10">
          ...
          <p className="text-sm text-text-muted mb-6">
            Résous chaque exercice sur ta machine, puis marque-le comme complété. Les niveaux sont indiqués par des étoiles ⭐.
          </p>
      ...
      <div className="flex justify-between items-center pt-6 border-t border-surface-3/30">
        {prevModule ? (
          <button onClick={() => navigate(`/python/module/${prevModule.id}`)} className="btn-secondary text-sm">
            ← {prevModule.title}
          </button>
        ) : <div />}
        {nextModule ? (
          <button onClick={() => navigate(`/python/module/${nextModule.id}`)} className="btn-primary text-sm">
            {nextModule.title} →
          </button>
        ) : (
          <Link to="/python" className="btn-primary text-sm">🎉 Parcours terminé !</Link>
        )}
      </div>
    </div>
  );
