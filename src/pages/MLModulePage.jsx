...
export default function MLModulePage() {
  ...
  const getNextML = useCallback((curModId, curExIdx) => {
    const curMod = ML_MODULES.find((m) => m.id === curModId);
    ...
    const curIdx = ML_MODULES.findIndex((m) => m.id === curModId);
    for (let i = curIdx + 1; i < ML_MODULES.length; i++) {
      if (ML_MODULES[i].exercises?.length > 0) return { moduleId: ML_MODULES[i].id, exerciseIndex: 0, moduleDone: true, allDone: false };
    }
    return { moduleId: null, exerciseIndex: null, moduleDone: true, allDone: true };
  }, []);

  const nextInfo = getNextML(mod.id, activeExercise);
  let nextLabel = 'Exercice suivant →';
  if (nextInfo?.allDone) nextLabel = '🎉 Parcours terminé !';
  else if (nextInfo?.moduleDone) {
    const nm = ML_MODULES.find((m) => m.id === nextInfo.moduleId);
    nextLabel = `Module suivant : ${nm?.title} →`;
  }

  const handleNext = useCallback(() => {
    if (!nextInfo) return;
    if (nextInfo.allDone) { navigate('/ml'); return; }
    if (nextInfo.moduleDone) { navigate(`/ml/module/${nextInfo.moduleId}`); }
  ...
  return (
    <div className="animate-fade-in">
      <Link to="/ml" className="inline-flex items-center gap-1.5 text-sm text-text-muted hover:text-text-secondary transition-colors mb-6">
      ...
      {mod.exercises?.length > 0 && (
        <section className="mb-10">
          <h2 className="font-display font-bold text-xl text-text-primary mb-1">🐍 Exercices pratiques — Python sur ta machine</h2>
          <p className="text-sm text-text-muted mb-4">
            Réalise chaque exercice dans Jupyter Notebook. Le code est fourni — copie, exécute, observe, comprends.
          </p>
