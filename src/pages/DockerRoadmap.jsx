    ...
    <div className="animate-fade-in">
      <div className="mb-10">
        ...
        <p className="text-text-secondary max-w-2xl mb-4">
          Du premier conteneur aux stacks de production. Une section installation pour
          bien démarrer, puis des exercices guidés que vous réalisez sur votre propre
          machine — avec indices et solutions à portée de main.
        </p>
      ...
      <div className="relative pl-8 sm:pl-10">
        <div className="absolute left-[14px] sm:left-[18px] top-0 bottom-0 w-0.5"
          style={{ background: `linear-gradient(180deg, ${DOCKER_MODULES.map((m) => m.colorHex).join(', ')})` }} />

        <div className="flex flex-col gap-6">
          {DOCKER_MODULES.map((mod, idx) => {
            ...
            return (
              <div key={mod.id} className="relative animate-slide-up"
                ...
                <Link to={`/docker/module/${mod.id}`} className="card-interactive p-5 block">
      ...
      {/* Capstone Project */}
      {TRACK.capstone && (
        <CapstoneProject trackId="docker" capstone={TRACK.capstone} />
      )}
    </div>
  );
}
